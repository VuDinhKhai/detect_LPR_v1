...
def add_camerasdetail(list,rtsp):
     ...
     if ID_list and ID_rtsp:
          ...
          camera_id = ID_rtsp[0]
          status = 0
          sql_insert_camera_detail = "INSERT INTO cameradetail (camera_id, list_vehicle_id,status) VALUES (%s, %s,%s)"
          id_detail = (camera_id, list_vehicle_id,status)
...
def remove_list(name_list):
     ...
     if ID_list is not None:
          ...
          sql = "SELECT vehicle_id FROM listvehicledetail WHERE list_vehicle_id = %s"
          values = (ID_list,)
          cursor.execute(sql, values)
          ID_lp = cursor.fetchall()
          if ID_lp is not None:
               for i in ID_lp:
                    idlp = i[0]
                    update_query = "UPDATE vehicles SET status = 0 WHERE id = %s"
                    cursor.execute(update_query, (idlp,))
                    db.commit()
          
          sql2 = "SELECT camera_id FROM cameradetail WHERE list_vehicle_id = %s"
          cursor.execute(sql2, values)
          ID_lp = cursor.fetchall()
          if ID_lp is not None:
               for i in ID_lp:
                    idlp = i[0]
                    update_query = "UPDATE cameras SET status = 0 WHERE id = %s"
                    cursor.execute(update_query, (idlp,))
                    db.commit()
...
def set_status_camera_view(rtsp , name_list):
     update_query = "UPDATE cameradetail SET status = 0"
     cursor.execute(update_query)
     db.commit()
     ID_camera_ = None
     ID_camera_ = get_id_camera_where_rtsp(rtsp)
     if ID_camera_ is not None:
          for ID_camera in ID_camera_:
               ID_list_ = None
               ID_list_ = get_full_id_From_listsvehicle_WHERE_name(name_list)
               if ID_list_ is not None:
                    for ID_list in ID_list_:
                         ID_camera = ID_camera[0]
                         ID_list = ID_list[0]
                         update_query1 = """
                         UPDATE cameradetail SET status = 1 WHERE camera_id = %s AND list_vehicle_id = %s
                         """
                         data = (ID_camera,ID_list)
                         cursor.execute(update_query1, data)
                         # Lưu thay đổi và đóng kết nối
                         db.commit()
     
     
def get_1_status_camera_view():
     db.commit()
     select_query = """
     SELECT table1.camera_id, table1.list_vehicle_id
     FROM cameradetail AS table1
     WHERE table1.status = 1
     """
     cursor.execute(select_query)
     results = cursor.fetchall()
     
     # Khai báo biến để lưu danh sách cặp (name, rtsp)
     name_rtsp_list = []
     
     if results is not None:
          for result in results:
               camera_id = result[0]
               list_vehicle_id = result[1]

               # Thực hiện truy vấn để lấy RTSP từ camera_id
               rtsp_query = f"""
               SELECT rtsp
               FROM cameras
               WHERE id = {camera_id};
               """
               cursor.execute(rtsp_query)
               rtsp_result = cursor.fetchone()
               if rtsp_result is not None:
                    rtsp = rtsp_result[0]  # Assuming the 'rtsp' column is the first column in the result
               else:
                    rtsp = None 

               # Thực hiện truy vấn để lấy name từ list_vehicle_id
               name_query = f"""
               SELECT name
               FROM listsvehicle
               WHERE id = {list_vehicle_id};
               """
               cursor.execute(name_query)
               name_result = cursor.fetchone()
               if name_result is not None:
                    name = name_result[0]  # Assuming the 'rtsp' column is the first column in the result
               else:
                    name = None 
               # Thêm cặp (name, rtsp) vào danh sách
               name_rtsp_list.append((name, rtsp))

     return name_rtsp_list
     

def get_id_vehicle_where_list_vehicle(list_vehicle):
     cursor = db.cursor(buffered=True)
     names = list_vehicle.split('-')
     list_vehicle_ids = []
     for name in names:
          # Truy vấn 'listvehicledetail' để lấy 'list_vehicle_id' từ 'name'
          query = f"SELECT id FROM listsvehicle WHERE name = '{name}'"
          cursor.execute(query)
          result = cursor.fetchone()
          if result:
               list_vehicle_ids.append(result[0])
     # list_vehicle_ids = list(map(str, list_vehicle.split('-')))
     selected_vehicle_ids = []
     if list_vehicle_ids:
          # Bước 3: Lấy danh sách 'vehicle_id' dựa trên 'list_vehicle_id'
          query = f"""
               SELECT vehicle_id
               FROM listvehicledetail
               WHERE list_vehicle_id IN ({','.join(map(str, list_vehicle_ids))})
          """
          cursor.execute(query)
          results = cursor.fetchall()

          for row in results:
               selected_vehicle_ids.append(row[0])
     if selected_vehicle_ids:
          count_dict = {}
          for num in selected_vehicle_ids:
          ...
          numbers_with_max_count = [num for num, count in count_dict.items() if count == max_count]
          if numbers_with_max_count:
               for num in numbers_with_max_count:
                    sql = "SELECT license_plate FROM vehicles WHERE status = 1 AND id = %s"
                    values = (num,)
                    cursor.execute(sql, values)
                    results = cursor.fetchall()
                    if results is not None:
