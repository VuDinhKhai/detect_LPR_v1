        ...
        if det is not None and len(det):
            ...
            if LP_type == "2":
                ...
                for l in sorted(center_list, key = lambda x: x[0]):
                    license_plate += str(l[2])
            # print("Licenseplate found : " + license_plate)
    print("Time Recognizing license plate " + str(time.time() - to) )
    to = time.time()
    return license_plate
def detect_recognition_letter_APP1(weights_reco, img0, device):
    to = time.time()
    half = device.type != 'cpu'
    model_reco = attempt_load(weights_reco, device=device)
    imgsz = check_img_size(512, s=model_reco.stride.max())
    ...
    img = img.float()  # uint8 to fp16/32
    img /= 255.0  # 0 - 255 to 0.0 - 1.0
    img = img.half() if half else img.float()  # Set data type
    img = img.unsqueeze(0)  # Add batch dimension
    # Inference
    pred = model_reco(img, augment=False)[0]
    # Apply NMS
    pred = non_max_suppression(pred, 0.65, 0.65, agnostic=False)
    license_plate = ""
    for i, det in enumerate(pred):
        # gn = torch.tensor(img0.shape)[[1, 0, 1, 0]]  # normalization gain whwh
        if det is not None and len(det):
...
def detect_APP(weights_detect,weights_reco,img0):
    ...
    # Apply NMS
    pred = non_max_suppression(pred, 0.7, 0.5, classes=0, agnostic=False)
    #print("pred " + str(pred))  pred [tensor([[250.17928,  95.72924, 264.18994, 105.58556,   0.41472,   0.00000]])]
    # Process detections
    for i, det in enumerate(pred):
        gn = torch.tensor(img0.shape)[[1, 0, 1, 0]]  # normalization gain whwh
        if det is not None and len(det):
            ...
            for *xyxy, conf, cls in reversed(det): # *xyxy : tọa độ bbox, conf : độ chính xác, cls : class mấy
                ...
                if conf > 0.7:
                    ...
                    print(" Biển số xác định được : " + str(value_LP))
                    plot_one_box(xyxy, img0, label=value_LP, color=colors[int(cls)], line_thickness=3)
        return img0,value_LP
    
def detect_APP1(weights_detect,weights_reco,img0):
    to =  time.time()
    device = select_device('')
    half = device.type != 'cpu'

    # Load model
    model_detect = attempt_load(weights_detect, device=device)
    imgsz = check_img_size(512, s=model_detect.stride.max())
    if half:
        model_detect.half()
    names = model_detect.module.names if hasattr(model_detect, 'module') else model_detect.names
    colors = [[random.randint(0, 255) for _ in range(3)] for _ in range(len(names))]
        ...
        if det is not None and len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                if conf > 0.7:
                    ...
                    crop = img0[y1:y2, x1:x2]
                    value_LP = detect_recognition_letter_APP1(weights_reco, crop, device)
                    print("License plate detected: " + str(value_LP))
                    plot_one_box(xyxy, img0, label=value_LP, color=colors[int(cls)], line_thickness=3)
    print("Time detected: " + str(time.time()-to))
    to  = time.time()
    return img0, value_LP
